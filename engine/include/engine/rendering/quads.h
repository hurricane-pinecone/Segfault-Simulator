...
// One particle billboard: four screen-space corners (already rotated/scaled),
// per-corner UVs (for sprite-sheet frames) and a single linear RGBA modulate
// colour. Unlit -- particles carry their own colour rather than sampling scene
// lighting.
struct ParticleQuad
{
  glm::vec2 points[4] = {};
  ...
  // Holds the world painter sort-key when the command is built; the render
  // system's assignClipDepth() remaps it in place to clip-space depth
  // (gl_Position.z) so each particle occludes against terrain individually.
  float z = 0.0f;
};
