...
/**
 * Flat 2D world-to-screen projection: a panned, zoomed orthographic view with
 * no elevation. World units map straight to pixels scaled by zoom, so the
 * elevation argument is ignored. This is the projection a plain 2D game (e.g. a
 * side-on platformer) renders through; IsometricProjection is the heightfield
 * variant.
 */
