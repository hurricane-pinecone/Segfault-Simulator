...
// beneath. The game's terrain generator is the ITerrainSurfaceSource. Holds no
// render state, so it lives with the particle engine rather than the renderer.
...
public:
  ...
  float groundElevation(glm::vec2 worldPos) const override
  {
    if (!m_terrain)
      return 0.0f;
    return static_cast<float>(m_terrain->terrainHeightAt(
        static_cast<int>(glm::floor(worldPos.x)),
        static_cast<int>(glm::floor(worldPos.y))));
  }

private:
  static constexpr int kMinWallStep = 1;

  static glm::ivec2 floorTile(const glm::vec2& p)
  {
    return {static_cast<int>(glm::floor(p.x)),
