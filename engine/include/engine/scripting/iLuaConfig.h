...
//   <name>.set{ ... } -> apply edits to the live object, then onLuaConfigChanged()
//   <name>.options    -> the schema doc (key -> hint) for autocomplete
//
...
{
public:
  virtual ~ILuaConfig() = default;

  // Global table name this config is exposed under (e.g. "sun").
  virtual std::string luaConfigName() const = 0;

  // The editable field surface. The fields' member offsets are taken relative to
  // luaConfigData().
