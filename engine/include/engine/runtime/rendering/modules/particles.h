 ...
 *
 * Register effects through the inherited ParticleEngine API; call enableStains()
 * once to make decal-leaving effects stick to the host's surface (no manual sink
 * or collision wiring).
 */
...
public:
  ...
  // Turn on persistent splatter: effects with leavesDecal now stick to a surface
  // and stamp a mark on it, using the host's decal sink. Pass a terrain
  // heightfield to stick to it (iso); pass nothing to stick to the scene's
  // colliders (flat). One call replaces wiring a sink + collision source by hand.
