...
@group(0) @binding(4) var<storage, read> desc : array<u32>; // per-slot (offset, dim)
...
// Body-voxel index uses the slot's class stride; the label buffers stay 96^3.
fn bIdx(slot : u32, x : i32, y : i32, z : i32) -> u32 {
  let bd = i32(desc[slot * 2u + 1u]);
  return desc[slot * 2u] + u32(x + y * bd + z * bd * bd);
}
fn solidAt(slot : u32, x : i32, y : i32, z : i32) -> bool {
  return (bodyVox[bIdx(slot, x, y, z)] & 3u) != 0u;
}
...
fn recolor(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  let g = (h >> 8u) & 255u;
  let b = h & 255u;
  bodyVox[bi] = (r << 24u) | (g << 16u) | (b << 8u) | (v & 0xFFu);
}
