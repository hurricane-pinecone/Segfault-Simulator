// Stamp a resting rigid body's solid voxels back into the world (both buffers)
// at its final transform, turning the fallen body into terrain again. Stamped at
// the 90-degree rest pose, which is a clean axis swap, so it leaves no holes.
struct Body {
  flags : vec4<u32>,
  invRot0 : vec4<f32>,
  invRot1 : vec4<f32>,
...
@group(0) @binding(3) var<uniform> body : Body;
...
fn stamp(@builtin(global_invocation_id) gid : vec3<u32>) {
  ...
  if ((v & 3u) != 1u) { return; } // solid voxels only
