...
void main()
{
  ...
  // Ported from the lit terrain shader, adapted for REAL geometry: the surface
  // normal is the real per-vertex normal (so "up" is normal.z, not the billboard
...
// but marched along the sun's azimuth against the heightmap and compared to the
// sun's fixed altitude. 1 = lit, 0 = shadowed. Analytic against the dense
// heightmap, so it self-shadows + climbs walls with no bias/acne/peter-panning
// and no gaps. Shadows are capped at kSunShadowMaxDist tiles (low sun fades out).
// Directional sun occlusion: the same horizon-angle test the point lights use,
...
void OpenGLQuadRenderer::flushCurrentPipeline()
{
  ...
  // Decals and geometry draw immediately (decals from persistent/dynamic buffers,
  // geometry per draw call), so there's nothing queued to flush.
  case Pipeline::Decal:
